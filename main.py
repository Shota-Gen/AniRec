...
def sortAnimeID():  # works, creates a sorted list of animeID by descending rating order
    ...
    cur.execute("SELECT DISTINCT anime_id FROM AnimeTable ORDER BY rating DESC LIMIT 100") # can change l8r
...
def searchName(name):
    ...
    conn.text_factory = lambda x: unicode(x, 'utf-8', 'ignore')
    print name
     #  have it so that it inputs a name and outputs a list where 
    cur.execute("SELECT DISTINCT anime_id from AnimeTable WHERE anime_name == ?", name)  # have to make it so that it returns a list instead of just matching one
    conn.commit()
...
def rec():
    if request.method == 'POST':
        ...
        conn.close()
        if not recommend:
            return redirect(url_for("searcherror"))
        else:
...
def search():
    if request.method == 'POST':
        conn = sqlite3.connect("anime.db")
        want = request.form['search']  # returns the string entered
        print want
        result = getAnimeInfo(searchName(want))
        print result
        conn.commit()
        conn.close()
        if not result:
